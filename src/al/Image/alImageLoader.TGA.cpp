...
static int ReadTGAHeader(alFileBuffer* fb, _tga_header_t* h) 
{
	...
	else
	{
		...
		fb->Seek(pos, SEEK_SET);

		// Indicate that the TGA header was invalid
		alLog::PrintWarning("TGA: bad file\n");
		return 0;
	}
}

static bool ReadTGA_RLE(unsigned char* buf, int size, int bpp, alFileBuffer* fb) 
{
...
alImageLoaderTGA::alImageLoaderTGA()
{
}
...
alImage* alImageLoaderTGA::Load(const char* fn)
{
	AL_ASSERT_ST(fn);
	...
	{
		alLog::PrintWarning("TGA: failed to open file\n");
	}
	alLog::PrintWarning("TGA: file [%s]\n", fn);
	return 0;
}

alImage* alImageLoaderTGA::Load(alFileBuffer* fb)
{
	AL_ASSERT_ST(fb);
	_tga_header_t h;
	...
	if (cmapsize > 0)
	{
		...
		cmap = (unsigned char*)alMemory::Malloc(cmapsize);
		if (cmap == NULL)
		{
			alLog::PrintWarning("TGA: bad file\n");
			return 0;
	...
	pix = (unsigned char*)alMemory::Malloc(pixsize2);
	if (pix == NULL)
	{
		if (cmap)
			alMemory::Free(cmap);
	...
	{
		if (!ReadTGA_RLE(pix, pixsize, bpp, fb))
		{
			alMemory::Free(cmap);
			alLog::PrintWarning("TGA: bad file\n");
			return 0;
...
bool alImageLoaderTGA::GetInfo(const char* fn, alImage* img)
{
	AL_ASSERT_ST(fn);
	AL_ASSERT_ST(img);
...
bool alImageLoaderTGA::GetInfo(alFileBuffer* fb, alImage* img)
{
	AL_ASSERT_ST(fb);
	AL_ASSERT_ST(img);
...
uint32_t alImageLoaderTGA::GetExtensionNum()
{
	return 1;
}

const char* alImageLoaderTGA::GetExtension(uint32_t i)
{
	return "tga";
}

bool alImageLoaderTGA::Save(alImage* img, const char* p)
{
	return false;
}
